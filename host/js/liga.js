$(function(){
	...
	function get_premier(value) {
		$liga_teams.append(`<li class="list-group-item"><a href="team.html?team=${value.strTeam}">${value.strTeam}</a></li>`);
	};

	function Es_description(value){
		$Es_description.append(`<li class="list-group-item">${value[2].strDescriptionES}</li>`);
		$FR_description.append(`<li class="list-group-item">${value[2].strDescriptionFR}</li>`);
		$link_list.append(`<li class="list-group-item"> Facebook: <a href="${value[2].strFacebook}">${value[2].strFacebook}</a></li>`);
		$link_list.append(`<li class="list-group-item"> Twitter: <a href="${value[2].strTwitter}">${value[2].strTwitter}</a></li>`);
		$link_list.append(`<li class="list-group-item"> Youtube: <a href="${value[2].strYoutube}">${value[2].strYoutube}</a></li>`);
	};

	$.ajax({
		type: 'GET',
		url: 'https://www.thesportsdb.com/api/v1/json/1/lookup_all_teams.php?id=4335',
	...
	$.ajax({
		type: 'GET',
		url: `https://www.thesportsdb.com/api/v1/json/1/search_all_leagues.php?c=Spain&s=Soccer`,
		success: function(data){
			$.each(data, function(key, value){
				Es_description(value);
			});	
		},
		error: function(e){
			alert('error loading');
	...
	$.ajax({
		type: 'GET',
		url: `https://www.thesportsdb.com/api/v1/json/1/lookuptable.php?l=4335&s=1920`,
		success: function(data){
			$.each(data, function(key, value){
				$.each(value, function(k, v){
					create_table(k,v);
	...
	$.ajax({
		type: 'GET',
		url: `https://www.thesportsdb.com/api/v1/json/1/eventsnextleague.php?id=4335`,
		success: function(data){
			$.each(data, function(key, value){
				$.each(value, function(k, v){
	...
	$.ajax({
		type: 'GET',
		url: `https://www.thesportsdb.com/api/v1/json/1/eventspastleague.php?id=4335`,
		success: function(data){
			$.each(data, function(key, value){
				$.each(value, function(k, v){
	...
	$.ajax({
		type: 'GET',
		url: `https://www.thesportsdb.com/api/v1/json/1/lookupleague.php?id=4335`,
		success: function(data){
			$.each(data, function(key, value){
				$.each(value, function(k, v){
					...
					$description.append(`<li class="list-group-item"><img src="${v.strFanart3}" height="auto" width="100%"></li>`);
				});
			});	
		},
		error: function(e){
